...
var AmazonDb, BinaryCompressor, BlobHandler, DynamoQueue, S3Queue, TextCompressor, async, defaultOptions, util, zlib,
...
DynamoQueue = (function() {
  ...
  DynamoQueue.prototype.push = function(fn, description, callback) {
    ...
    return this.queue.push(fn, function(error, results) {
      var capacity, elapsed;
      if (_this.timing) {
        ...
        console.log('Dynamo[' + elapsed + 'ms,' + capacity + '] ' + _this.name + ': ' + description);
      }
...
S3Queue = (function() {
  ...
  S3Queue.prototype.push = function(fn, description, callback) {
    ...
    return this.queue.push(fn, function(error, results) {
      ...
      if (_this.timing) {
        elapsed = new Date().getTime() - start;
        console.log('S3[' + elapsed + 'ms] ' + _this.name + ': ' + description);
      }
      return callback(error, results);
    });
  };
...
module.exports = AmazonDb = function(options) {
  var client, db, dynamo, k, operations_ro_queue, operations_rw_queue, operations_table, s3, s3_ro_queue, s3_rw_queue, snapshots_bucket, snapshots_ro_queue, snapshots_rw_queue, snapshots_table, v, _ref;
      ...
      write_data: [
        'compress_data', function(cb, results) {
          ...
          return s3_rw_queue.push(function(c) {
            return s3.put(path, headers, results.compress_data, c);
          }, 'write Snapshot(' + docName + '-' + docData.v + ')', cb);
        }
      ]
    }, function(error, results) {
      if (error != null) {
        if ((error.message != null) && error.message.match('The conditional request failed')) {
    ...
    return async.auto({
      ...
      delete_snapshots: [
        'list_snapshots', function(cb, results) {
          if (results.list_snapshots.Count === 0) {
            return cb('Document does not exist', null);
          }
          return async.mapSeries(results.list_snapshots.Items, function(item, cb) {
            var request;
            request = {
              TableName: snapshots_table,
              Key: {
      ...
      delete_s3_snapshots: [
        'list_snapshots', function(cb, results) {
          ...
          return async.forEachSeries(results.list_snapshots.Items, function(item, cb) {
            return s3_rw_queue.push(function(c) {
              return s3.del('/' + snapshots_bucket + '/' + item.doc.S + '-' + item.v.N + '.snapshot', c);
            }, 'delete Snapshot(' + item.doc.S + '-' + item.v.N + ')', cb);
          }, function(error) {
            if (error != null) {
              return cb(error, null);
      ...
      delete_operations: [
        'list_operations', function(cb, results) {
          if (results.list_operations.Count === 0) {
            return cb(null, {});
          }
          return async.forEachSeries(results.list_operations.Items, function(item, cb) {
            var request;
            request = {
              TableName: operations_table,
              Key: {
    ...
    return async.auto({
      ...
      get_data: [
        'get_snapshot', function(cb, results) {
          var item;
          if (results.get_snapshot.Count !== 1) {
            return cb('Document does not exist', null);
          }
          item = results.get_snapshot.Items[0];
          return s3_ro_queue.push(function(c) {
            return s3.get('/' + snapshots_bucket + '/' + item.doc.S + '-' + item.v.N + '.snapshot', 'buffer', c);
      ...
      snapshot: [
        'compressor', function(cb, results) {
          return results.compressor.binary.decode(results.get_data.buffer, cb);
        }
      ],
      meta: [
        'compressor', function(cb, results) {
          return results.compressor.text.decode(results.get_snapshot.Items[0].meta.S, cb);
        }
      ]
    }, function(error, results) {
  ...
  this.getOps = function(docName, start, end, callback) {
    ...
    return async.auto({
      ...
      if (error != null) {
        ...
        data = [];
        return async.map(results.get_metadata.Items, function(operation, cb) {
          var compressor;
          compressor = new TextCompressor(operation.c != null, options['timing']);
          return async.auto({
            snapshot: function(c) {
              return compressor.decode(operation.op.S, c);
            },
            meta: function(c) {
